...
export async function POST(request: Request) {
  ...
  const rows = validateApplicationCsv(parsed.data.csv);
  ...
  const { data, error } = await auth.supabase.rpc("commit_application_import", { p_rows: rows.map((row) => ({ ...row.value as object, fingerprint: row.fingerprint })) });
